...
/// Cüzdan hareketlerinde running bakiye'yi (id sırasına göre) yeniden hesaplar.
/// Hareket silindikten sonra çağrılmalıdır.
pub async fn recompute_cuzdan_bakiyeleri(pool: &PgPool, hissedar_id: i64) -> sqlx::Result<f64> {
    #[derive(sqlx::FromRow)]
    struct Row { id: i64, borc: f64, alacak: f64 }
    let rows: Vec<Row> = sqlx::query_as(
        "SELECT id, borc, alacak FROM hissedar_cuzdanlari
         WHERE hissedar_id = $1 ORDER BY id ASC",
    )
    .bind(hissedar_id)
    .fetch_all(pool)
    .await?;
    ...
    for r in &rows {
        running += r.alacak - r.borc;
        sqlx::query("UPDATE hissedar_cuzdanlari SET bakiye = $1 WHERE id = $2")
            .bind(running)
            .bind(r.id)
            .execute(pool)
            .await?;
...
/// Cüzdana alacak kaydı ekler — operation_id ile damgalar (iptal için)
pub async fn cuzdan_alacak_ekle_op(
    pool: &PgPool,
    hissedar_id: i64,
    donem_id: Option<i64>,
    tarih: chrono::NaiveDate,
    ...
    let yeni = onceki + tutar;
    sqlx::query(
        "INSERT INTO hissedar_cuzdanlari (hissedar_id, donem_id, tarih, bilgi, borc, alacak, bakiye, operation_id)
...
/// Kasaya giren kaydı + operation_id damgası (iptal için)
pub async fn kasa_giren_ekle_op(
    pool: &PgPool,
    kasa_id: i64,
    tarih: chrono::NaiveDate,
    aciklama: &str,
    ...
    sqlx::query(
        "INSERT INTO kasa_hareketleri (kasa_id, tarih, aciklama, giren, cikan, bakiye, operation_id)
